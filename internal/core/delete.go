...
func (s *Service) DeleteEvent(id string) error {
	...
	if e.SourceUID != "" && s.CalendarConfigured() &&
		(e.Status == StatusCancelled || e.VanishedAt.IsZero()) {
...
//
// Who confirmed or cancelled what stays readable in audit.jsonl, the
// same place the proof of pruned deliveries lives.
func (s *Service) DeletePerson(id string) error {
	...
	for _, a := range s.state.Assignments {
		...
		if e == nil || e.Status == StatusCancelled || eventEnd(e).Before(now) {
			continue
		}
	...
	dropped := map[string]any{
		...
		"responses":          dropWhere(&s.state.Responses, func(r Response) bool { return r.PersonID == id }),
