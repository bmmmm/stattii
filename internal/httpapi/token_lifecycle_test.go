...
func TestAdminRotatePortalButton(t *testing.T) {
	...
	if w := ui.post(t, "/admin/people/"+p.ID+"/rotate-portal", nil); w.Code != http.StatusSeeOther {
...
func TestAdminRevokeLinksButton(t *testing.T) {
	...
	if w := ui.post(t, "/admin/event/"+e.ID+"/links/revoke", form); w.Code != http.StatusSeeOther {
