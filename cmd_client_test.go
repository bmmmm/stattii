...
func TestClientDispatchTable(t *testing.T) {
	...
	cases := []struct {
		...
		{[]string{"person", "telegram-link", "pe_1"}, "POST", "/api/v1/people/pe_1/telegram-link", nil},
...
func TestClientUsageStrings(t *testing.T) {
	cases := []struct {
		...
		{[]string{"person"}, "usage: stattii person list|add|set|test|rotate-portal|telegram-link|rm"},
		...
		{[]string{"person", "telegram-link"}, "usage: stattii person telegram-link <person-id>"},
